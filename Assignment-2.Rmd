...
p_load(data.table, dtplyr, dplyr, ggplot2, knitr, kableExtra)
```
...
shr_final = shr[shr$additional_victim_count <= 3 & !is.na(fips_state_code),]
...
state_pop_murder_synthetic = tibble(state_pop_murder_final) %>%
  filter(!(state_fips %in% c(6, 41, 32, 2)))

# Create a synthetic Colorado and Washington (matching on lagged murder rates)

# Need to create weights, w

# Min pre treatment murder rate

state_pop_murder_synthetic_2000_2013 = state_pop_murder_synthetic %%
  filter()
