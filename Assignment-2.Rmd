...
We fail to reject the null that marijuana did not increase crime. This is because the ratio of PostMSE/PreMSE is low and that implies that crime did not spike post treatment. Altogether this suggests that marijuana legalization does not increase crime.
...
output[state_fips_treated == 8, rnk]/(nrow(output)-1)

# Washington Empirical P Value
output[state_fips_treated == 53, rnk]/(nrow(output)-1)
```
...
No, the QTE's do not allow you to recover the distribution of individual treatment effects; they return the average treatment effect over the distribution of outcomes. The individual treatment effects are not recoverable since we must know both Y0 and Y1.
