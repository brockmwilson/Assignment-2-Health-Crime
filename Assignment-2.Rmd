...
co_dataprep = dataprep(
  ...
  treatment.identifier = 8, # integer that indicates the id variable value for our treatment unit
  controls.identifier =  unique(state_pop_murder_synthetic[!(state_fips %in% c(8,53)), state_fips]), # vector that indicates the id variable values for the donor pool
...
wa_dataprep = dataprep(
  ...
  unit.names.variable = "state", # identify our name variable
  time.variable = "year", # identify our time period variable
  treatment.identifier = 53, # integer that indicates the id variable value for our treatment unit
  controls.identifier =  unique(state_pop_murder_synthetic[!(state_fips %in% c(8,53)), state_fips]), # vector that indicates the id variable values for the donor pool
  time.optimize.ssr = c(2001:2014), # identify the time period you want to optimize over to find the W*. Includes pre-treatment period and the treatment year.
  time.plot = c(2001:2017) # periods over which results are to be plotted with Synth's plot functions
)
...
synth_out_co = co_dataprep %>% synth()
synth_out_wa = wa_dataprep %>% synth()
...
synth_out_co %>% path.plot(dataprep.res = co_dataprep, tr.intake = 2014)
synth_out_wa %>% path.plot(dataprep.res = wa_dataprep, tr.intake = 2014)
...
synth_control_co = co_dataprep$Y0plot %*% synth_out_co$solution.w
synth_control_wa = wa_dataprep$Y0plot %*% synth_out_wa$solution.w
